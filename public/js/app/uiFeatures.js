  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        resolve({ primary: 'rgb(248, 250, 252)', secondary: 'rgb(161, 161, 170)', tertiary: 'rgb(113, 113, 122)' });
      }
    };
...
function analyzeColors(imageData) {
  ...
  const luminance = (c) => (c.r * 299 + c.g * 587 + c.b * 114) / 1000;
  const mix = (a, b, t) => ({
    r: a.r + (b.r - a.r) * t,
    g: a.g + (b.g - a.g) * t,
    b: a.b + (b.b - a.b) * t
  });
  const mute = (c, amount = 0.35) => {
    const gray = (c.r + c.g + c.b) / 3;
    return {
      r: gray + (c.r - gray) * (1 - amount),
      g: gray + (c.g - gray) * (1 - amount),
      b: gray + (c.b - gray) * (1 - amount)
    };
  };
  const ensureMinLum = (c, minLum = 120) => {
    const lum = luminance(c);
    if (lum >= minLum) return c;
    // Mix toward white enough to reach the min luminance
    const t = clamp((minLum - lum) / 140, 0, 0.85);
  ...
  if (sortedColors.length === 0) {
    return { primary: 'rgb(248, 250, 252)', secondary: 'rgb(161, 161, 170)', tertiary: 'rgb(113, 113, 122)' };
  }

  // Pick 3 reasonably distinct colors
  let primary = sortedColors[0];
  if (luminance(primary) < 50 && sortedColors.length > 1) primary = sortedColors[1];

  const pickDistinct = (candidates, against, minDist) => {
    for (const c of candidates) {
      if (against.every((a) => distance(c, a) >= minDist)) return c;
    }
    return null;
  };

  const secondaryCandidate = pickDistinct(sortedColors, [primary], 70) || sortedColors[1] || primary;
  const tertiaryCandidate = pickDistinct(sortedColors, [primary, secondaryCandidate], 70) || sortedColors[2] || secondaryCandidate;

  // Mute + ensure visibility on dark UI
  const primaryOut = ensureMinLum(mute(primary, 0.42), 135);
  const secondaryOut = ensureMinLum(mute(secondaryCandidate, 0.50), 120);
  const tertiaryOut = ensureMinLum(mute(tertiaryCandidate, 0.58), 105);

  return {
    primary: toRgbString(primaryOut),
    secondary: toRgbString(secondaryOut),
    tertiary: toRgbString(tertiaryOut)
  };
}

export function applyDynamicColors(colors) {
  // Settings: allow users to disable dynamic colors.
  try {
    const raw = localStorage.getItem('player0.dynamicColorsEnabled.v1');
    if (raw) {
  ...
  root.style.setProperty('--dynamic-accent', colors.primary);
  root.style.setProperty('--dynamic-muted', `color-mix(in srgb, ${colors.tertiary} 22%, transparent)`);
...
// ============================================
export function initUIFeatures() {
  // Keep UI flat (no album-art blur layer behind the app).
  // This can look like a gradient/overlay even when gradients are removed.
  initViewTransitions();
  initContextMenus();
  initSearchFilters();
  ...
  document.addEventListener('songChanged', async (e) => {
    const song = e.detail;
    if (song && song.id) {
      // If disabled, make sure any prior session's class isn't sticking.
      try {
        const raw = localStorage.getItem('player0.dynamicColorsEnabled.v1');
  ...
  console.log('[UI] Initialized optional UI features');
}
